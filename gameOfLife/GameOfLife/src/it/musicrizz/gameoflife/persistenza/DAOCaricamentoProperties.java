    ...
    public  Sistema carica(File file) throws DAOException  {
        ...
        try{
            ...
            for(int i=0;i<numCellule;i++)   {
                ...
                //sistema.setCellulaStatoCorrente(x, y, true);//per diminuire il numero di messaggi notificaModificaCellula
                sistema.addCellula(x, y, true, false);
                log.debug("Cellula settata con pos -> "+x+","+y);
            }
            return sistema;
        }catch(Exception e)   {
            throw new DAOException("Errore nel caricamento del file /n"+file+" --> "+e);
        }
    }
    ...
    public Sistema carica(String file) throws DAOException  {
        ...
        try{
            ...
            for(int i=0;i<numCellule;i++)   {
                ...
                int y = Integer.parseInt(p.getProperty("Cellula"+i+"Y").trim());
                log.debug("Siamo nel for -> X="+x+" Y="+y+" Cellula Num= "+i);
                //sistema.setCellulaStatoCorrente(x, y, true);//per diminuire il numero di messaggi notificaModificaProprieta
                sistema.addCellula(x, y, true, false);
                log.debug("Cellula settata con pos -> "+x+","+y);
            }
            return sistema;
